const i18n_ru = {
    "translation": {
        "newVersionAvailable": "Установлена новая версия. Прочтите документацию перед использованием. Версия до обновления: ",
        ...
        "magicMemoryIsEnabled": "Память ВКЛ",
        "magicMemoryIsDisabled": "Память ВЫКЛ",
        ...
        "readme_part_1": `<div>
            ...
            <li>Скопировать ссылку&nbsp;<a id="link" onclick="event.preventDefault()"><strong>LINK</strong></a> которая будет служить адресом подключения для телефона <strong>КЛИЕНТА</strong>. В этом адресе указан ваш уникальный постоянный идентификатор. Поэтому если вы измените этот <strong>ID</strong> то вам придется исправить и сделать заново все следующие шаги.</li>
        ...
        "readme_part_6": `<p style="text-align: center;"><strong>(.)&nbsp;Memory</strong></p>
            <p>Эта функция включается с помощью символа разделения дробной части числа, которая находится между <strong>0</strong> и <strong>=</strong>. Все функции форсирования работают постоянно при включенной функции памяти. Даже если перезапустить приложение. После включения/выключения все форсирования сбрасываются. Таким образом после включения функции памяти требуется также включить ту или иную функцию форсирования на ваше усмотрение.</p>`,
        "readme": `<p style="text-align: center;"><strong>Авторское право</strong></p>
            <p style="text-align: justify;">Весь контент в этом веб приложении защищен законом об авторских правах и может использоваться только с разрешения <a href="mailto:stark1912@gmail.com">Mikhail Tsiuzhou "Michael Stark"</a> Любое несанкционированное копирование, распространение, модификация и использование веб приложения является нарушением авторских прав. Для получения разрешения на использование приложения или если вы получили приложение от других источников - <a class="inline" href="mailto:stark1912@gmail.com">немедленно сообщите мне</a>, таким образом поддержав авторский продукт.</p>
            ...
            <p style="text-align: justify;">Что бы приложение запросило необходимые разрешения у операционной системы, нужно нажать в любом месте экрана.</p>
            <p style="text-align: justify;"><span style="text-decoration: underline;">Android</span>: разрешение для отображения уведомлений. Если разрешить, то функция "история ввода" будет отображаться в панели уведомлений. Таким образом можно получать уведомления на смарт часах. В противном случае - информация будет отображаться во всплывающем окне.</p>
            <p style="text-align: justify;"><span style="text-decoration: underline;">iOS</span>: разрешение на доступ к ориентации устройства для работы функции "DD форс". Начиная с iOS 13 приложение обязано запрашивать разрешение при каждом запуске приложения. Запрос на разрешение будет происходить только при включенной функции "DD форс".</p>
            ...
            <p style="text-align: center;">Добавлена возможность постоянного форсирования - Функция <strong>(.) Memory</strong>.</p>
            <p style="text-align: center;"><strong>1.62</strong></p>
            <p style="text-align: center;">Исправлена индикация нажатой операции после того, как включен ручной режим переворота устройства экраном вниз. Apple передумала отключать PWA приложения.</p>
